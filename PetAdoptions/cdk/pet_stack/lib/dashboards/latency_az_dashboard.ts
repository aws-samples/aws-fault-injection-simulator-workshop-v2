        ...
        const body = {
            widgets: [
                ...
                logWidget('petlistadoptions — p99 latency by AZ',
                    ...
                    12, 2, 12, 6),
                logWidget('petsearch — p99 latency by AZ (FIS AZ-slowdown target)',
                    "SOURCE '/ecs/PetSearchEc2' | parse @message /az=(?<az>[a-z0-9-]*)/ | parse @message /latency_ms=(?<lat>\\d+)/ | filter ispresent(lat) | stats pct(lat,99) as p99_ms, avg(lat) as avg_ms by az, bin(1m)",
                    0, 8, 12, 6),
